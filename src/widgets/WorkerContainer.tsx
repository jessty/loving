...
export default class WorkerContainer extends WorkerContainerBase<WorkerContainerProperties> {
	...
	private _createFilterLinks() {
		...
		for (let i = 0; i < alphabet.length; i++) {
			...
			links.push(
					...
					<span classes={this.theme(css.links)}>
						<Link key={char} to='filter' params={{filter: char}}>{char}</Link>
					</span>
	...
	protected render() {
		...
		<Worker key={`worker-${i}`} {...worker}></Worker>);

		return (
			<div>
				<h1 classes={this.theme(css.title)}>Worker Directory</h1>
				<div classes={this.theme(css.filters)}>
					{this._createFilterLinks()}
				</div>
